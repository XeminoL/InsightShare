...
| 3 | Gọi Rekognition `detect_labels` (MaxLabels=10, MinConfidence=55) gắn nhãn ảnh khi upload; lưu nhãn để tìm kiếm. | 28/07/2026 | 28/07/2026 | |
| 4 | Gọi Textract `detect_document_text` trích text từ PDF và ảnh scan; file `.txt` đọc thẳng từ object trên S3. | 29/07/2026 | 29/07/2026 | |
| 5 | Thêm endpoint `POST /files/{id}/ask`: gửi văn bản đã trích kèm câu hỏi tới một model Claude trên Amazon Bedrock và trả về câu trả lời theo ngôn ngữ câu hỏi; không có câu hỏi thì tóm tắt. | 30/07/2026 | 30/07/2026 | [Bedrock](https://docs.aws.amazon.com/bedrock/) |
...
1. Ảnh upload được tự gắn nhãn bằng Rekognition, còn văn bản tài liệu đi qua nhánh Textract, cả hai đều lưu theo owner của file.
