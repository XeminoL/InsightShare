...
| --- | --- | --- | --- | --- |
| Mon | Write a deploy script (`deploy-lambda.ps1`: zip the function, run `aws lambda update-function-code`, wait for the update) so re-deploying after a code change is one command. | 10/08/2026 | 10/08/2026 |  |
...
1. Deploy and teardown are scripted (`deploy-lambda.ps1` and `cleanup-aws.ps1`), so re-deploying is one command and teardown leaves only the CloudFront distribution to disable by hand.
