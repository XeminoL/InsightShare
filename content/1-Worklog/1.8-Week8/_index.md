...
| Thu | Add the `POST /files/{id}/ask` endpoint: send the extracted text plus a question to a Claude model on Amazon Bedrock (`invoke_model`) and return the answer in Vietnamese; with no question, summarize instead. | 23/07/2026 | 23/07/2026 | [Bedrock](https://docs.aws.amazon.com/bedrock/) |
...
2. A document Q&A endpoint answers questions and summarizes in Vietnamese via Bedrock/Claude; it fails soft with an HTTP 200 message when Model access is not enabled.
