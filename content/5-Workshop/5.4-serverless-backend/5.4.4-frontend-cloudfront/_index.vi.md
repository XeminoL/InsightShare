...
Giao diện là một file `index.html` tĩnh (HTML/CSS/JS thuần): upload file, hiển thị danh sách kèm nhãn AI, có ô tìm kiếm theo nội dung, và tạo link chia sẻ. Nó chỉ nói chuyện với endpoint API Gateway, nên cùng một trang chạy được cả ở local lẫn trên CloudFront.
...
Một tệp mở qua link chia sẻ công khai:

![Trang chia sẻ công khai](/images/5-Workshop/5.4-serverless-backend/web-share.png)
...
- Các bước này chạy trực tiếp từ trình duyệt trên trang web đang hoạt động, nên upload, phân tích AI và tìm kiếm theo nội dung đều chạy đầu-cuối trên AWS.
