...
One **Python Lambda function** for InsightShare's back-end. A single function dispatches all routes by HTTP method and path: upload, list, search, analyze, get, delete.
...
def handler(event, context):
    ...
    if len(parts) == 3 and parts[2] == "analyze" and method == "POST":
        return analyze(event, parts[1])  # -> AI layer (section 5.4.5)
