...
Sản phẩm chính là **InsightShare**, ứng dụng web upload và chia sẻ ảnh/tài liệu theo hướng serverless trên AWS. Công việc trải qua S3, Lambda, API Gateway, DynamoDB, Cognito, CloudFront, IAM, CloudWatch, các dịch vụ AI Rekognition, Textract và Bedrock (Claude), Python (boto3), cùng tài liệu kỹ thuật song ngữ.
...
| 7 | **Tư duy giải quyết vấn đề** | x | | Chẩn đoán các lỗi tích hợp (CORS, IAM, credentials bị cache trên Lambda còn ấm, S3 307, `Decimal` của DynamoDB) từ log CloudWatch. |
| 8 | **Đóng góp cho dự án** | x | | Tự làm InsightShare end-to-end và bổ sung tính năng thêm (xóa file, link có hạn, cô lập dữ liệu theo người dùng). |
...
* Các lệnh gọi AI (Textract, Bedrock) nên bọc lại và có đường lùi, để một dịch vụ không sẵn sàng chỉ làm mất phần phân tích chứ luồng upload vẫn xong.
